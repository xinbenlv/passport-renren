 ...
 * 
 * This update, uses the V2.0 API: http://wiki.dev.renren.com/wiki/API_new
 *
...
function Strategy(options, verify) {
  ...
  this._userURL = options.userURL || 'https://api.renren.com/v2/user/get';
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    //For some reason, RenRen doesn't return the complete User Info for the logged in user. We use the id to call the API again
    //that does return more information.
    var loggedUser = JSON.parse(body);

    self._oauth2.get(self._userURL + '?userId=' + loggedUser.response.id, accessToken, function (err, body, res) {
      if (err) { return done(new InternalOAuthError('Failed to fetch user profile - Step 2', err)); }
